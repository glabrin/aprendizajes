        ...
        <div class="collapse" id="general-pages">
          <ul class="nav flex-column sub-menu">
            @foreach (Session::get('espacios') as $espacio)
                <li class="nav-item"> <a class="nav-link" href="{{ url('espacio',['nombre'=>$espacio->nombre]) }}">{{ str_replace('_', " ", ucfirst($espacio->nombre)) }}</a></li>
            @endforeach
          </ul>
        </div>
    </li>
...
<div class="card-columns">
  @foreach (Session::get('espacios') as $espacio)
    <div class="card text-white bg-dark">
        ...
        @if ($espacio->año == 1)
          <?php $año = "1er" ?>
        @elseif ($espacio->año == 2)
          <?php $año = "2do" ?>
        @elseif ($espacio->año == 3)
          <?php $año = "3ro" ?>
        @elseif ($espacio->año == 4)
          <?php $año = "4to" ?>
        @elseif ($espacio->año == 5)
          <?php $año = "5to" ?>
        @else
        <?php $año = "6to" ?>
        @endif
        ...
        @if ($espacio->año <= 3)
          <?php $ciclo = "Ciclo básico" ?>
        @else
        <?php $ciclo = "Ciclo orientado" ?>
        @endif

          <h5 class="card-title">{{str_replace('_', " ", ucfirst($espacio->nombre))}}</h5>
          <p class="card-text"><small class="text-muted">{{$año}} año - {{$ciclo}}</small></p>
        </div>
      </div>
    @endforeach
</div>
